...
const DEFAULT_DATA: ApplicationData = {
  step1: {
    loanType: 'personal', amount: 0, tenureMonths: 0, loanPurpose: '',
  },
  personal: {
    fullName: '',
    dateOfBirth: '',
  ...
  kyc: {
    pan: '', panStatus: 'idle', aadhaar: '', aadhaarStatus: 'idle', aadhaarConsent: false,
  },
  address: {
    current: {
      line1: '', pinCode: '', city: '', state: '', residenceType: 'owned', yearsAtAddress: 0,
    },
  ...
  documents: { documents: [] },
  consents: {
    accuracy: false, creditCheck: false, terms: false, communications: false,
  },
};
  ...
  const patch = useCallback(
    (key: string, partial: Record<string, unknown>) => {
      setState((prev) => {
        ...
        const nextData = {
          ...prev.data,
          [key]: { ...(currentSlice as Record<string, unknown>), ...partial },
        } as ApplicationData;
