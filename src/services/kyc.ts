...
export async function verifyPan(pan: string, loanType: LoanType): Promise<KycResult> {
  ...
  if (!validation.valid) {
    return { field: 'pan', status: 'rejected', message: validation.message, checkedAt }
  }
  ...
  return { field: 'pan', status: 'verified', message: 'PAN verified successfully.', checkedAt }
}
...
export async function verifyAadhaar(aadhaar: string): Promise<KycResult> {
  ...
  return { field: 'aadhaar', status: 'verified', message: 'Aadhaar verified successfully.', checkedAt }
}
